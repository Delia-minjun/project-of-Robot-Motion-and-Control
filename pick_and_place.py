...
#         gripper_joint_names = rospy.get_param(rospy.get_namespace() + "gripper_joint_names", ["finger_joint"])
#         self.gripper_joint_name = gripper_joint_names[0]

#       rospy.loginfo("Pick and Place Task Initialized")
...
#     val = relative_position * (gripper_max_absolute_pos - gripper_min_absolute_pos) + gripper_min_absolute_pos
#     return self.gripper_group.go([val], wait=True)
...
#   # --- 参数定义 (请根据你的实际环境修改 x, y, z) ---
#   # 抓取点：假设物体在前方 40cm，中心线上
#   pick_x, pick_y, pick_z = 0.4, 0.0, 0.12 
#   # 放置点：假设在右侧 20cm
#   place_x, place_y, place_z = 0.3, 0.2, 0.12
...
#   success &= task.reach_gripper_position(0.0) # 0是全开

#   # 2. 移动到抓取位上方 (Pre-Pick)
#   rospy.loginfo("Step 2: Moving to Pre-Pick position")
#   # 姿态：俯视 (Roll=pi, Pitch=0, Yaw=0)
#   pre_pick_pose = task.create_pose(pick_x, pick_y, pick_z + 0.1, pi, 0, 0)
#   success &= task.reach_cartesian_pose(pre_pick_pose)

#   # 3. 下降并抓取
#   rospy.loginfo("Step 3: Picking the object")
#   pick_pose = task.create_pose(pick_x, pick_y, pick_z, pi, 0, 0)
#   success &= task.reach_cartesian_pose(pick_pose)
#   time.sleep(0.5)
#   success &= task.reach_gripper_position(0.7) # 0.7为闭合一部分抓紧物体

#   # 4. 提升物体 (Post-Pick)
#   rospy.loginfo("Step 4: Lifting the object")
#   success &= task.reach_cartesian_pose(pre_pick_pose)

#   # 5. 移动到放置位上方 (Pre-Place)
#   # 注意：这里 Yaw 改为 pi/4 (45度)，满足任务要求 5：Pick 和 Place 姿态不同
#   rospy.loginfo("Step 5: Moving to Place position with DIFFERENT POSE")
#   pre_place_pose = task.create_pose(place_x, place_y, place_z + 0.1, pi, 0, pi/4)
#   success &= task.reach_cartesian_pose(pre_place_pose)

#   # 6. 下降并放置
#   rospy.loginfo("Step 6: Placing the object")
#   place_pose = task.create_pose(place_x, place_y, place_z, pi, 0, pi/4)
#   success &= task.reach_cartesian_pose(place_pose)
#   success &= task.reach_gripper_position(0.0) # 打开夹爪释放
